...
CloseTab(TabIndex, Confirm := True, Multiple := False, Exiting := False) {
    Local SkipConfirmation, Result, TabCount, IsActiveTab, NewIndex, FullPath
...
GetCurrentWord(ByRef Word, Pos := -1) {
    Local n, CurrentPos, PrevChar, WordStartPos, WordEndPos
...
GetWordPos(n) {
    Local Pos, WordStartPos, PrevChar, WordEndPos
...
OnWM_NOTIFY(wParam, lParam, msg, hWnd, Obj) {
    Local n, CurPos, Line, LexType, BracePos, BraceMatch, Indentation, WordPos, Calltip, Word, Keyword
...
*/
        ...
        If (g_AutoComplete && g_oAutoC[LexType].bLoaded) {
            ShowAutoCList(n, g_AutoCMinLength)
        }

        ; Calltips
        If (Obj.Ch == 40 || Obj.Ch == 44 || Obj.Ch == 32) { ; "(", "," and " ".
            If (g_CalltipTyping && g_oAutoC[LexType].bLoaded) {
        ...
        If (g_CalltipAutoC && Keyword != "GuiClose") {
            Calltip := GetCalltip(Keyword, True)
            ShowCalltip(n, Calltip, GetWordPos(n)[1])
        }
...
LoadAutoComplete(Type) {
    Local Keys, Key, Name, BaseName, oXML, aList := []
    ...
    oXML := LoadXML(A_ScriptDir . "\AutoComplete\" . BaseName . ".xml")
    If (!IsObject(oXML)) {
        Return 0
    }
    ...
    For Key in Keys {
        aList.Push(Key.getAttribute("name"))
    }

    g_oAutoC[Type] := {}
    g_oAutoC[Type].aList := aList
...
CreateAutoCList(aList, Filter) {
    Local Item, Out := "", Len := StrLen(Filter)

    Loop % aList.Length() {
        Item := aList[A_index]
        If (SubStr(Item, 1, Len) = Filter) {
            Out .= Item . " "
        }
    }
...
ShowAutoCList(n, MinLength := 3) {
    Local CurPos, WordStartPos, LengthEntered, Type, PrevChar, aTypeList, Filter, AutoCList ;, WordFirstChar

    CurPos := Sci[n].GetCurrentPos()
    WordStartPos := Sci[n].WordStartPosition(CurPos, False) ; True = onlyWordCharacters
    LengthEntered := CurPos - WordStartPos
    If (Sci[n].AutoCActive() || LengthEntered < MinLength) {
        Return 0
    }

    ;WordFirstChar := Sci[n].GetCharAt(WordStartPos)
    ...
    aTypeList := g_oAutoC[Type].aList
    Filter := GetTextRange(n, WordStartPos, CurPos)
    AutoCList := CreateAutoCList(aTypeList, Filter)
...
GetCalltip(Keyword, Overload := True) {
    Local n, oNode, LineText, Params, Calltip, Separator, aList, Type

    n := TabEx.GetSel()
    Type := Sci[n].Type

    aList := g_oAutoC[Type].aList
    Loop % aList.Length() {
        If (aList[A_Index] = Keyword) {
            Keyword := aList[A_Index]
            Break
        }
    }
...
ShowCalltipEx(n := 0, Position := -1, Overload := True, Desc := False) {
    Local Word, WordPos, Calltip
    If (!n) {
        n := TabEx.GetSel()
    }

    WordPos := GetCurrentWord(Word, Position)
    Calltip := GetCalltip(Word, Overload)
    ShowCalltip(n, Calltip, WordPos[1])
}

InsertCalltip() {
    Local n := TabEx.GetSel(), EndPos, Word, Calltip, NextChar
...
ShowCaretDialog:
    ...
    SetWindowIcon(hCaretDlg, IconLib, -37)
    Gui Font, s9, Segoe UI
    Gui Color, White
...
GetSelectionPos(n, ByRef StartPos, ByRef EndPos, Index := 1) {
    If (Index == 0) {
...
ShowSyntaxDialog() {
    Global
    Local LexTypes, hWndSyntax, Key, LexType, Syntax, Keywords, AutoC, oXML, ThemeFile, AutoCFile, Loaded, Checked
    ...
    Gui Add, Pic, x-2 y-2 w697 h51, % "HBITMAP:" . CreateGradient(697, 51)
    ...
    Gui Add, ListView, hWndhLvSyntax x11 y93 w673 h256 -Multi +Checked +LV0x14000
    ...
    Gui Add, Button, gSyntaxDlgSubmit x434 y399 w80 h23 +Default, &OK
    Gui Add, Button, gSyntaxDlgReset x520 y399 w80 h23, &Reset
    Gui Add, Button, gSyntaxDlgClose x606 y399 w80 h23, &Cancel

    GuiControl -Redraw, %hLvSyntax%
    For Key, LexType in g_oLexTypes {
        ...
        If (oXML.url != "") {
            ...
            Try {
                Syntax := oXML.selectNodes("/scheme/theme[1]").length() ? " ✔" : ""
                Keywords := oXML.selectNodes("/scheme/keywords/language[@id='" . Key . "']").length() ? " ✔" : ""
            }
        }

        AutoCFile := A_ScriptDir  . "\AutoComplete\" . LexType.Name . ".xml"
        oXML := LoadXML(AutoCFile)
        If (oXML.url != "") {
            Try {
                AutoC := oXML.selectNodes("/AutoComplete/language[@id='" . Key . "']").length() ? " ✔" : ""
            }
        }
    ...
    SetExplorerTheme(hLvSyntax)
    GuiControl +Redraw, %hLvSyntax%
    LV_ModifyCol(7, "AutoHdr")
}

SyntaxDlgSubmit() {
    Local Row, Name, LexType, n, LexTypes

    Gui SyntaxDlg: Default
    Gui Submit, NoHide
    Gui ListView, %hLvSyntax%

    Row := LV_GetNext()
    ; Change current and default LexType
    If (Row) {
        LV_GetText(Name, Row)
