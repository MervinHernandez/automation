...
; Main window
Gui Main: New, LabelOn hWndg_hWndMain Resize MinSize680 -DPIScale, %g_AppName% v%g_Version%
Gui Main: Default

; Startup boost: minimum initialization of menu items
AddMenu("MenuFile", "&New File`tCtrl+N", "NewTab", IconLib, -2)
...
Menu MenuOptions, Add, Enable &AutoComplete, ToggleAutoComplete
...
CreateToolbar() {
    ...
    IL_Add(TB_IL, IconLib, -66)  ; Debug Run
    IL_Add(TB_IL, IconLib, -69)  ; Debug Stop
    IL_Add(TB_IL, IconLib, -72)  ; Step Into
    IL_Add(TB_IL, IconLib, -73)  ; Step Over
    IL_Add(TB_IL, IconLib, -74)  ; Step Out
    IL_Add(TB_IL, IconLib, -78)  ; Inspect Variables
    ...
    g_hToolbar := ToolbarCreate("ToolbarHandler", TB_Btns, TB_IL, "FLAT LIST TOOLTIPS", Extra, "", 65536)
}
...
OnWM_KEYDOWN(wParam, lParam, msg, hWnd) {
    ...
    } Else If (hWndActive == g_hWndVarList) {
        If (wParam == 13) {
            ModifyVariable()
            Return False
        }
    ...
    ; AHK debugger
        ...
        If (wParam == 116) { ; F5
            SetTimer, DebugRun, -1 ; RunWait
            Return 0

        } Else If (wParam == 117) { ; F6
            ShiftP ? StepOut() : StepInto()
...
OpenFolder(StartPath := "", FileManager := "", Params := "") {
    ...
    If (FileExist(FileManager) && !GetKeyState("Shift", "P")) {
        If (Params != "") {
            FileManager := FileManager . " " . Params
        }

        ;OutputDebug %FileManager%
        RunEx(FileManager . " " . StartPath)
...
CustomMessage(wParam, lParam) {
    ...
    If (wParam == 1 && WinExist("ahk_id " . lParam)) {
        ...
        TabIndex := IsFileOpened(Params[1])
        If (TabIndex) {
            TabEx.SetSel(TabIndex)
        } Else {
            If (!OpenFileEx(Params[1])) {
                Return
            }
        }
...
ShowBackupDialog() {
    ...
    Gui Add, Pic, x-2 y-2 w556 h51, % "HBITMAP:" . CreateGradient(556, 51)
...
AhkRunGetStdErr(n, AhkPath, AhkScript, Parameters, WorkingDir, AhkDbgParams := "") {
    Local CmdLine, StdErr, ExitCode, Marked, AhkStdErr, File, Line
...
EscPressed() {
    ...
    If (g_AutoComplete && Sci[n].AutoCActive()) {
        Sci[n].AutoCCancel()
        g_AutoComplete := False
        SetTimer EnableAutoComplete, -3000
        Return False
    }
...
UpdateMenuState(hMenu) {
    ...
    If (MenuName == "View") {
        ...
        CheckMenuItem("MenuOptions", "Enable &AutoComplete", g_AutoComplete)
...
ShowSaveSessionDialog() { ; M
    ...
    Gui Add, Pic, x-2 y-2 w774 h51, % "HBITMAP:" . CreateGradient(774, 51)
    Gui Font, s12 cWhite, Segoe UI
...
SaveSessionDialog_InvertSelection() {
    Gui Session: Default
    Gui ListView, %g_hLvSessionFiles%
    Loop % LV_GetCount() {
...
ShowCmdLineParamsDialog() { ; M
    ...
    Gui Add, Pic, x-2 y-2 w618 h51, % "HBITMAP:" . CreateGradient(618, 51)

    If (Executable != "") {
        Gui Add, Picture, x573 y6 w32 h32 +BackgroundTrans, % "HICON:" . GetFileIcon(Executable, False)
        Gui Add, Picture, x587 y27 w16 h16 +BackgroundTrans +Icon2, % "HICON:" . GetFileIcon(FullPath)
    }
    Gui Font, s12 cWhite, Segoe UI
...
ShowFileHistoryManager(ActiveTab := 1) {
    ...
    SetWindowIcon(g_hWndFHM, IconLib, -37)

    Gui Add, Pic, x-2 y-2 w702 h51, % "HBITMAP:" . CreateGradient(702, 51)
    Gui Font, s12 cWhite, Segoe UI
    ...
    Gui Tab, 1 ; Sessions
    ToolbarCreate("FHM_OnToolbar", Buttons1,, TB_Options,, TB_Size)
    ...
    Gui Tab, 2 ; Favorites
    ToolbarCreate("FHM_OnToolbar", Buttons2,, TB_Options,, TB_Size)
    Gui Add, ListView, hWndg_hLvFavorites gFHM_LvHandler %LV_Options%, Favorites

    Gui Tab, 3 ; Recent
    ToolbarCreate("FHM_OnToolbar", Buttons3,, TB_Options,, TB_Size)
    Gui Add, ListView, hWndg_hLvRecentFiles gFHM_LvHandler %LV_Options%, Recent

    Gui Tab, 4 ; Open Files
    ToolbarCreate("FHM_OnToolbar", Buttons4,, TB_Options,, TB_Size)
...
FHM_LvHandler(hWnd, Event, Row, Error := "") {
    Local TabText
...
FHM_OnToolbar(hWnd, Event, Text, Pos, Id) {
...
ShowAhkSettings() {
    ...
    Gui Add, Pic, hWndhPic x-2 y-2 w642 h51, % "HBITMAP:" . CreateGradient(642, 51)
    Gui Font, s12 cWhite, Segoe UI
...
CreateButton(TB_Handler, IconRes, IconIndex, PosSize, Tooltip, Options := "Flat List NoDivider Tooltips Tabstop") {
    ...
    Return ToolbarCreate(TB_Handler, Tooltip, hIL, Options,, PosSize)
}

CreateGradient(Width, Height) {
    Return CreateDIB(g_Gradient, 2, 2, DPIScale(Width), DPIScale(Height), 1)
}
